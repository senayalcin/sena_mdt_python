...
#	foreach my $key1(sort keys %relEnstExpress) {
#		foreach my $key2(sort keys %{$relEnstExpress{$key1}}) {
#			foreach my $key3(sort keys %{$relEnstExpress{$key1}->{$key2}}) {
#				print("$key1\t$key2\t$key3\t$relEnstExpress{$key1}->{$key2}->{$key3}\n");
#			}
#		}
#	}

#		foreach my $v (@allRelEnstExpress) {
#			print("$v\n");
#		}
...
#	foreach my $key1(sort keys %enrichment) {
#		foreach my $key2(sort keys %{$enrichment{$key1}}) {
#			foreach my $key3(sort keys %{$enrichment{$key1}->{$key2}}) {
#				print("$key1\t$key2\t$key3\t$enrichment{$key1}->{$key2}->{$key3}\n");
#			}
#		}
#	}

#		foreach my $v (@allEnrichment) {
#			print("$v\n");
#		}
...
#	foreach my $key(sort keys %canon) {
#		print("$key\t$canon{$key}\n");
#	}
...
#	foreach my $key1(sort keys %missInt4isoform) {
#		foreach my $key2(sort keys %{$missInt4isoform{$key1}}) {
#			print("$key1\t$key2\t$missInt4isoform{$key1}->{$key2}\n");
#		}
#	}

	#foreach my $key(sort keys %intN) {
	#	print("$key\t$intN{$key}\n");
	#}
...
#	foreach my $enstRed (sort keys %red) {-- g asagıdaki cld , hash elemanını önce degıskene alıyor. altakinde direkt yazıyor.
#     my $enstSel = $red{$enstRed};
#     print "$enstRed\t$enstSel\n";
 #  }

#	foreach my $key(sort keys %red) {
#		print("$key\t$red{$key}\n");
#	}
    return \%red;
}
...
#	foreach my $v ($med) {---- G Asagıdaki cld
#     print "$v\n";
#    }

#	print("$med\n");-- tek değer olduğu için 
